...
real browser. Every level runs automatically in GitHub Actions on every push.
...
| Data model | The EF Core model creates a valid schema for SQLite and SQL Server | xUnit, EF Core | `tests/LearnHub.Tests/Data` |
...
| Database provider | The same 182 tests on SQL Server 2022, plus applying all migrations to an empty database | Docker SQL Server in CI | `ci.yml` job "SQL Server migrations, tests and smoke test" |
...
| Smoke | A published build (and, after deployment, the live site) serves pages, links, protected areas and security headers | `scripts/smoke-test.sh` | `ci.yml`, `deploy.yml` |
...
Test data is isolated: each integration test class gets its own in-memory SQLite database (or its own throw-away SQL
Server database) with migrations and demo data applied, so tests can run in parallel and never touch a developer's
database. Test accounts use synthetic passwords defined in the test factory; CI generates random passwords for the
browser tests.
...
97 test methods produce 182 test cases, because theories run once per data row.

| Class | Methods | Covers |
|-------|---------|--------|
| `Unit/LogicTests` (grader, display format, progress, view model validation) | 15 | Scoring and pass mark (rounding down, unanswered questions, option ids from other questions), progress percentages, line colours, registration rules (strong matching passwords, accepted terms), question rules (two options, one correct, no duplicates), category icon allow-list |
| `Unit/ContentSafetyTests` (video parser, lesson renderer, search pattern) | 10 | YouTube and Vimeo URLs accepted only in known formats, HTML in lesson text is encoded, only allow-listed formatting is produced, code blocks stay encoded, attacks through formatting markers stay text, LIKE wildcards escaped, long search terms truncated |
| `Unit/FileStorageServiceTests` | 7 | Valid image saved with a random name and detected type, mismatched content rejected, oversize files rejected even with a small declared length, PDFs accepted for documents, only uploaded thumbnails can be deleted |
| `Data/DatabaseModelTests` | 3 | SQLite schema creation, SQL Server create script generation, intentional delete behaviour of key relationships (Restrict and Cascade) |
| `Services/CatalogueServiceTests` | 8 | Duplicate category names (case-insensitive), category deletion rules, search and filters return published courses only, unpublished details hidden except for admins, double enrolment and unpublished enrolment rejected, progress counting, leaving a course keeps completions |
| `Services/QuizAndCourseManagementTests` | 7 | Graded attempts store every answer, other students' results are hidden, non-enrolled students cannot take quizzes, quiz pages never contain correct answers, foreign option ids are not modified, removing a selected answer keeps attempts, course deletion order |
| `Integration/PublicSiteTests` | 14 | Public pages load, only published courses listed, server-side search, category filter, course details, friendly 404s, drafts hidden, preview lessons open while others need login, private files protected, security headers, health endpoint, static assets, home map lines, isolated test database |
| `Integration/AuthenticationTests` | 9 | Registration (role, sign-in, password hashing), server-side validation, duplicate email, role-based redirect after login, generic wrong-password error, local-only return URLs, logout, lockout after five failures, antiforgery rejection |
| `Integration/AuthorizationTests` | 7 | Access matrix for guest, student and admin across protected pages, 403 access denied page, students cannot post to admin actions even with a valid token |
| `Integration/StudentJourneyTests` | 4 | Register, enrol, study, take the quiz and see the result; tampered answers graded on the server; leaving removes lesson access; profile validation and saving |
...
### Browser scenarios (each runs on desktop 1440×900, tablet 768×1024 and Pixel 7)
...
| `student.spec.js` | Dashboard, course progress, lesson, quiz and result |
...
| `accessibility.spec.js` | Student pages: dashboard, My Courses, lesson, quiz results, profile, change password |
...
| TC-06 | Search | Search "sql" | Only "Relational Database Design with SQL", "1 course found" | As expected | Pass |
...
| TC-09 | Quiz tampering | Submit an option id from another question | Answer counted as incorrect | As expected | Pass |
...
| TC-17 | Course deletion | Delete a course with attempts | Answers, attempts, quizzes, lessons and enrolments removed; no database error | As expected on SQLite and SQL Server | Pass |
...
| 3 | Link crawl test | `/Admin/Courses/Details/{id}` returned 500 on SQLite | The query needed SQL `APPLY`, which SQLite cannot translate | Rewrote it as separate flat queries | `945aed8` |
...
| 5 | SQL Server CI job | Form pages returned 500 in Production over plain HTTP | Secure-only antiforgery cookie refused without HTTPS | Enabled forwarded headers, as App Service needs, and made the smoke test send `X-Forwarded-Proto` | `3851e07` |
...
| Production | Health, HTTPS redirect, admin login, upload and student journey on Azure | To do | Run the verification table in [Deployment](Deployment.md#5-verify-production) after provisioning |
