...
| | |
|---|---|
| Scope | The LearnHub application (`src/LearnHub`), its configuration, the CI/CD workflows, `infra/provision.sh` and the repository contents |
...
| Secrets | No passwords or keys in Git; user secrets in development; App Service settings in production; Azure SQL through managed identity; GitHub deploys through OpenID Connect | `appsettings.json` (empty seed passwords), `infra/provision.sh`, `.github/workflows/deploy.yml` |
...
| A02 Security Misconfiguration | Production defaults: HTTPS, HSTS, secure cookies, CSP and headers, no developer pages; App Service with FTPS disabled and TLS 1.2 | `PublicSiteTests.Html_responses_carry_security_headers`; smoke test header checks in CI and after deployment | Addressed |
...
| A08 Software or Data Integrity Failures | Antiforgery tokens on every POST; uploads verified by content signature; deployments only from GitHub Actions through OIDC | `AuthenticationTests.Posts_without_an_anti_forgery_token_are_rejected`; `FileStorageServiceTests.Rejects_a_file_whose_content_does_not_match_its_extension`; `Rejects_files_over_the_size_limit_even_if_the_declared_length_is_small` | Addressed |
| A09 Security Logging and Alerting Failures | Structured logs for sign-in failures, lockouts, seeding, administrator creation, enrolments, deletions and discarded spam; App Service log stream | Logs checked in CI: the SQL Server and browser jobs fail if the application logs errors | Partly addressed (see 4) |
...
- Development passwords are stored with `dotnet user-secrets` in the developer's profile.
- Production settings live in App Service; the SQL connection uses `Authentication=Active Directory Managed Identity`
  and contains no password; the administrator password is typed into `infra/provision.sh` and never printed.
- GitHub Actions deploys with OpenID Connect federation; there is no publish profile or client secret in the repository.
- CI generates throw-away passwords for its SQL Server container and browser-test accounts at run time and masks
  them in the logs. The only fixed passwords in the repository are synthetic ones in
...
| Monitoring and alerting | Logs are written but no alerts are configured | Connect App Service to Application Insights with alerts on failed requests, 5xx responses and lockouts |
| Rate limiting per instance | In-memory limiter; resets on restart and is per instance | Acceptable for the single-instance deployment; use Azure Front Door or a distributed limiter if scaled out |
| Azure SQL network access | "Allow Azure services" firewall rule plus Entra-only authentication | Use a virtual network with a private endpoint on paid tiers |
| Uploaded PDFs | Signature-checked but not malware-scanned | Scan uploads with Microsoft Defender for Storage if files move to Azure Storage |
