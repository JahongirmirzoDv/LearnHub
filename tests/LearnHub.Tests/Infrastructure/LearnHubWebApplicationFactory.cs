...
/// <summary>
/// Runs the real LearnHub application in memory. Every factory instance gets its own in-memory SQLite database
/// (migrations and demo seeding really run) and its own temporary file storage, so test classes can run in
...
public sealed class LearnHubWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    public const string DemoStudentEmail = "student@learnhub.test";
    ...
    {
        ConnectionString = $"Data Source=learnhub-tests-{Guid.NewGuid():N};Mode=Memory;Cache=Shared";
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.UseSetting($"ConnectionStrings:{DatabaseOptions.ConnectionStringName}", ConnectionString);
