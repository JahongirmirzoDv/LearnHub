...
/// <summary>
/// Runs the real LearnHub application in memory. Every factory instance gets its own in-memory SQLite
/// database (migrations and demo seeding really run) and its own temporary file storage, so test classes
...
{
    // Test-only credentials for throw-away in-memory databases.
    ...
    private readonly string _connectionString = $"Data Source=learnhub-tests-{Guid.NewGuid():N};Mode=Memory;Cache=Shared";
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.UseSetting("ConnectionStrings:Sqlite", _connectionString);
