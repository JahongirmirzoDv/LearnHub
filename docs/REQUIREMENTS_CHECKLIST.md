...
stable ID. The final audit ([REQUIREMENT_TRACEABILITY.md](REQUIREMENT_TRACEABILITY.md)) maps each ID to its implementation,
evidence and status.

Legend: **M** = mandatory (assignment brief), **O** = optional improvement.
**✅** = implemented and verified against the code; **PENDING** = built and documented, but the hosting step itself has
not been performed yet (see §14).

Every requirement below was re-checked against the code on the `feature/railway-firebase-sqlite` branch. The measured
evidence behind the ticks is **211 automated tests passing**, **42 browser checks passing** (14 Playwright tests ×
desktop, tablet and mobile) and a seeded catalogue of **8 categories, 12 courses, 64 learning resources, 11 quizzes,
54 questions and 216 answer options**. File-level implementation and evidence for each ID are in
...
| TECH-04 | One database provider (SQLite) with code-first EF Core migrations | M | ✅ |
| TECH-05 | ASP.NET Core Identity for authentication | M | ✅ |
| TECH-06 | Razor views, HTML5 semantic markup | M | ✅ |
| TECH-07 | CSS3 with Bootstrap 5.3 customised by a project design system | M | ✅ |
...
| DB-08 | Realistic seed data (8 categories, 12 courses, 64 resources, 11 quizzes, demo activity) | M | ✅ |
...
| CI-02 | GitHub Actions container workflow that reproduces the Railway runtime | M | ✅ |
| DEP-01 | Production ASP.NET application packaged and ready to run on Railway | M | PENDING |
| DEP-02 | SQLite production database on a persistent volume, configured through environment variables | M | PENDING |
| DEP-03 | Documented migration strategy and production verification | M | ✅ |
| PAGES-01 | Separate static presentation site with an "Open Learning System" call to action | M | ✅ |
| PAGES-02 | Presentation site deployment automated and reproducible | M | ✅ |
...
## 14. Notes on partial or pending items

Everything above is either verified in the code or listed here. No requirement is claimed that the code does not
support.

| ID | Note |
|----|------|
| DEP-01, DEP-02 | The Docker image, `railway.json`, the environment-variable contract and `scripts/set-app-url.sh` are complete, and the `container` CI job runs the image the way Railway does, including a restart that proves the SQLite file on `/data` survives. The Railway CLI is installed but **not authenticated**, so no Railway deployment has been made and no Railway URL is claimed. |
| ADM-09 | The three attempt pages exist and are crawled and scanned by the test suite, but there is no dedicated automated test for deleting a single attempt. |
| TEST-04 | [TESTING.md](TESTING.md) §7 records 13 passed checks. Keyboard-only journeys, a screen-reader pass, real devices and the live production check remain "To do" because they need a person or a deployment. |
| UI-06 | Automated axe-core WCAG 2.2 A/AA scans pass on the public, student and admin pages at all three viewports. This does not replace the manual keyboard and screen-reader pass above. |
| PAGES-01 | The presentation site is live at <https://learnhub-wapp.web.app>. Its buttons carry the conventional Railway address until `scripts/set-app-url.sh` rewrites it after the first deployment. |
| GIT-03 | `README.md` has all required sections, but its badges, hosting table and deployment section still describe the earlier Azure hosting and are not yet updated. |
